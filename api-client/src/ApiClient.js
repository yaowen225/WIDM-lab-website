...
class ApiClient {
   ...
   deserialize(data, returnType) {
    ...
    if (Array.isArray(data)) {
      return data.map((item) => this.deserialize(item, returnType))
    } else if (typeof data === 'object') {
      if (returnType && returnType.constructFromObject) {
        return returnType.constructFromObject(data)
      }
      return data
    }
    return data
        ...
        request.end((error, response) => {
            if (callback) {
              ...
              if (response && response.body) {
                console.log('API 响应原始数据:', response.body); // 调试信息
                data = this.deserialize(response.body, returnType);
              } else {
                console.error('API 响应为空或没有返回 body:', response); // 调试信息
              }
